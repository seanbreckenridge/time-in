...
def iter_countries() -> Iterator[str]:
    """
    Create a fzf-searchable list of countries.
    """
    for ctry in countries:
        for tz in ctry["timezones"]:
            yield f"""{ctry["continent"]}/{ctry["name"]} ({ctry["code"]}) [{ctry["capital"]}] - {tz}"""


def _get_tz_from_fzf_line(picked_fzf: str) -> zoneinfo.ZoneInfo:
    """
    Get the timezone name from the fzf output.
    """
    if " - " in picked_fzf:
        return zoneinfo.ZoneInfo(picked_fzf.rsplit(" - ")[-1])
    else:
        return zoneinfo.ZoneInfo(picked_fzf)
